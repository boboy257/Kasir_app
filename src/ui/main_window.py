...
class MainWindow(QMainWindow):
    def __init__(self, on_logout=None):
        ...
        self.btn_riwayat.setStyleSheet("""
            QPushButton { 
            ...
            QPushButton:focus { border: 2px solid #FFB74D; }
        """)
        self.btn_riwayat.clicked.connect(self.buka_riwayat)
        self.btn_riwayat.setAutoDefault(True)
        self.btn_riwayat.installEventFilter(self)
        
        # Tombol Pengaturan (Admin Only)
    ...
    def set_user_role(self, username, role):
        ...
        if role == "kasir":
            ...
            # Sembunyikan menu selain Mode Kasir
            tombol_dilarang = [1, 2, 3, 5, 6, 7]  # Index menu yang disembunyikan
        ...
        if event.type() == QEvent.Type.KeyPress:
            ...
            if current_index is not None:
                ...
                if event.key() == Qt.Key.Key_Right:
                    if (current_index + 1) < len(self.buttons) and self.buttons[current_index + 1].isVisible(): 
                        next_index = current_index + 1
                        
                elif event.key() == Qt.Key.Key_Left:
                    if (current_index - 1) >= 0 and self.buttons[current_index - 1].isVisible(): 
                        next_index = current_index - 1
